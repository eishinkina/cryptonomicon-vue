  ...
  <div class="container">
    ...
    <template v-if="tickers.length > 0">
      ...
      <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
        <div v-for="t in filteredTickers()" :key="t.name" @click="select(t)" :class="{
          'border-4': sel === t
    ...
    <section class="relative" v-if="sel">
      ...
      <div class="flex items-end border-gray-600 border-b border-l h-64">
        <div v-for="(bar, i) in normalizeGraph()" :key="i" :style="{ height: `${bar}%` }"
  ...
  methods: {
    ...
    subscribeToUpdates(tickerName) {
      setInterval(async () => {
        ...
        if (typeof data.USD !== 'undefined') { // Добавлено условие проверки
          ...
          console.warn(`No USD value for ${tickerName}`);
        }
      }, 3000);
      this.ticker = "";
      this.filter = "";
